...
public class DataCleaningService {
  ...
  /**
   * Extract endpoints from ALREADY-EXTRACTED code chunks using AI to parse into
   * structured JSON.
   * 
   * CRITICAL: The code chunks have already been extracted via semantic search.
   * This method only PARSES and STRUCTURES the extracted code, it does NOT
   * discover new endpoints.
   * 
   * @param codeChunks Already-extracted code from semantic search (contains route
   *                   definitions)
   * @param provider   AI provider configuration (Groq, OpenAI, etc.)
   * @return List of ExtractedEndpoint objects parsed from code (may contain
   *         duplicates)
   */
        ...
        try {
          ...
          if (isRateLimit && attempt < maxRetries - 1) {
            ...
            try {
              ...
              log.error("[DataCleaning] Retry interrupted");
              return null;
            }
          } else {
   ...
   * It only cleans, deduplicates, and standardizes what has already been
   * extracted.
   * 
   * @param extractedSections List of already-extracted README sections from code
   *                          chunks
